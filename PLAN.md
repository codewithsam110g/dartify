...
| 1.4 | **Tier A — represent properly.** Order revised by the S1.3 census: `this` (**900 sites**), then **`typeof x` (443 sites — the census's surprise; it was ranked minor because the audit counted *files*, not occurrences)**, then `readonly T[]`, qualified names, type predicates, named/optional tuple members, bare `null` → `Null` | `P-07`, `T-01`, `T-07`, `T-10` |
